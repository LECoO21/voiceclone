...
enum ProbeConfiguration {
    ...
    private static var isPortable: Bool {
        Bundle.main.object(forInfoDictionaryKey: "VoiceClonePortableRuntime") as? Bool ?? false
    }
    ...
    static var applicationSupportDirectory: URL {
        if let override = ProcessInfo.processInfo.environment["VOICECLONE_APP_SUPPORT"],
           !override.isEmpty {
    ...
    static var outputDirectory: URL {
        if let override = ProcessInfo.processInfo.environment["VOICECLONE_OUTPUT_DIR"],
           !override.isEmpty {
            return URL(fileURLWithPath: override, isDirectory: true)
        }
        let musicDirectory = FileManager.default.urls(for: .musicDirectory, in: .userDomainMask)[0]
        return musicDirectory.appendingPathComponent("VoiceClone", isDirectory: true)
    }
