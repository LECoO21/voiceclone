...
APP_PATH="${OUTPUT_ROOT}/VoiceClone.app"
LEGACY_APP_PATH="${OUTPUT_ROOT}/LocalAudioProbe.app"
PREVIOUS_APP_PATH="${OUTPUT_ROOT}/声音导演.app"
...
if [[ "${APP_PATH}" != "${OUTPUT_ROOT}/VoiceClone.app" ]]; then
  print -u2 -- "拒绝清理意外的应用路径：${APP_PATH}"
  exit 2
fi
rm -rf "${APP_PATH}"
for legacy_path in "${LEGACY_APP_PATH}" "${PREVIOUS_APP_PATH}"; do
  case "${legacy_path}" in
    "${OUTPUT_ROOT}/LocalAudioProbe.app"|"${OUTPUT_ROOT}/声音导演.app")
      rm -rf "${legacy_path}"
      ;;
  esac
done
...
ICON_TEMP_ROOT="$(mktemp -d "/tmp/voice-clone-icon.XXXXXX")"
...
if [[ "${ICON_TEMP_ROOT}" == /tmp/voice-clone-icon.* ]]; then
  rm -rf "${ICON_TEMP_ROOT}"
fi

if [[ "${BUILD_FLAVOR}" == "portable" ]]; then
  plutil -insert VoiceClonePortableRuntime -bool true "${CONTENTS_PATH}/Info.plist"
