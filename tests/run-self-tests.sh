...
run_test() {
  ...
  VOICECLONE_TEST_ROOT="${test_root}" \
  VOICECLONE_APP_SUPPORT="${test_root}/ApplicationSupport" \
  VOICECLONE_OUTPUT_DIR="${test_root}/Output" \
  VOICECLONE_TEST_FIXTURES="${TESTS_DIR}/fixtures" \
  VOICECLONE_PROJECT_DIR="${PROJECT_DIR}" \
    "${test_root}/${test_name}"
}
...
if [[ "${VOICECLONE_SKIP_PLAYBACK_TEST:-0}" == "1" ]]; then
